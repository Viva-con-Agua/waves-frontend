<template>
  <div style="margin-top:40px;width:50%;margin:auto">
    <el-row class="mst-fvd-item" style="padding:10px" v-for="pe in getRecomandations" :key="pe.id">
...
export default {
  name: "RecomandationView",
  components: {},
  data() {
    return {};
  },
  methods: {
    ...mapActions({ fetchRecomandations: "FETCH_RECOMANDATIONS" }),
    calculateMonth(date) {
      let d = new Date(date);
  ...
  mounted() {
    this.fetchRecomandations();
    console.log(this.getRecomandations);
  },
  computed: {
    ...mapGetters(["getRecomandations"])
  }
};
</script>
